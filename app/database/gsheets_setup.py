...
def _initialize_expected_structure():
    ...
    # Убедимся, что URI и имена колонок загружены
    if app_config.GSHEETS_USERS_URI and app_config.COL_USER_TG_ID:
        EXPECTED_SHEETS_STRUCTURE[app_config.GSHEETS_USERS_URI] = (
            _extract_sheet_name_from_uri(app_config.GSHEETS_USERS_URI),
            0,  # Временный gid, будет обновлен при проверке
            [app_config.COL_USER_TG_ID, app_config.COL_USER_PRIVACY_ACCEPTED]
        )
    if app_config.GSHEETS_CATEGORIES_URI and app_config.COL_CATEGORY_NAME:
        EXPECTED_SHEETS_STRUCTURE[app_config.GSHEETS_CATEGORIES_URI] = (
            _extract_sheet_name_from_uri(app_config.GSHEETS_CATEGORIES_URI),
            0,  # Временный gid, будет обновлен при проверке
            [app_config.COL_CATEGORY_NAME]
        )
    if app_config.GSHEETS_ITEMS_URI and all([
        app_config.COL_ITEM_ID, app_config.COL_ITEM_CATEGORY_NAME,
        app_config.COL_ITEM_NAME, app_config.COL_ITEM_DESCRIPTION, app_config.COL_ITEM_PRICE,
        app_config.COL_ITEM_IMAGE_URL, app_config.COL_ITEM_UNIT,
        app_config.COL_ITEM_QUANTITY, app_config.COL_ITEM_STATUS,
    ...
    if not EXPECTED_SHEETS_STRUCTURE:
        logger.warning("Структура ожидаемых Google Sheets не была инициализирована. Проверьте конфигурацию URI и имен колонок.")
...
def _extract_sheet_name_from_uri(uri: str) -> str:
    ...
    try:
        ...
        logger.error(f"Не удалось извлечь имя листа из URI: {uri}. Убедитесь, что URI содержит '#sheet=ИмяЛиста'.")
...
async def ensure_google_sheets_setup():
    ...
    if not EXPECTED_SHEETS_STRUCTURE:
        logger.error("Не удалось определить структуру таблиц из конфигурации. Настройка прервана.")
        return

    logger.info("Запуск проверки и настройки структуры Google Sheets...")
    gs_client = get_gspread_client()
    if not gs_client:
        logger.error("Не удалось получить gspread клиент. Настройка Google Sheets прервана.")
        return
    ...
    for spreadsheet_id, sheet_configs in spreadsheets_map.items():
        try:
            ...
            logger.info(f"Открыта таблица: '{spreadsheet.title}' (ID: {spreadsheet_id})")
            ...
            for sheet_name, headers in sheet_configs:
                ...
                if sheet_name in existing_worksheets_titles:
                    logger.info(f"Лист '{sheet_name}' уже существует в таблице '{spreadsheet.title}'.")
                    ...
                    logger.info(f"Лист '{sheet_name}' не найден. Создание листа в таблице '{spreadsheet.title}'...")
                    try:
                        # Создаем лист с достаточным количеством строк и колонок (можно настроить)
                        worksheet = spreadsheet.add_worksheet(title=sheet_name, rows="100", cols=str(max(20, len(headers))))
                        # Сохраняем gid нового листа
                        gid = worksheet.id
                        SHEET_GIDS[spreadsheet_id + "#" + sheet_name] = gid
                        logger.info(f"Лист '{sheet_name}' успешно создан с gid {gid}")
                    except Exception as e_add:
                        logger.error(f"Не удалось создать лист '{sheet_name}': {e_add}", exc_info=True)
                ...
                if worksheet:
                    ...
                    current_headers = []
                    try:
                        current_headers = worksheet.row_values(1) # Получаем значения первой строки
                        ...
                        if "exceeds grid limits" in str(e_api).lower() or "Unable to parse range" in str(e_api):
                            logger.info(f"Лист '{sheet_name}' пуст или имеет неопределенные границы, заголовки будут записаны.")
                            current_headers = []
                        else:
                            logger.error(f"API ошибка при чтении заголовков с листа '{sheet_name}': {e_api}")
                            continue # Пропустить этот лист

                    # Удаляем пустые строки или None в конце списка заголовков, которые может вернуть gspread
                    cleaned_current_headers = [h for h in current_headers if h is not None and str(h).strip() != ""]

                    if cleaned_current_headers != headers:
                        logger.info(f"Заголовки на листе '{sheet_name}' ({cleaned_current_headers}) "
                        ...
                        try:
                            ...
                            logger.info(f"Заголовки для листа '{sheet_name}' успешно записаны/обновлены.")
                        except Exception as e_update:
                            logger.error(f"Не удалось записать/обновить заголовки для листа '{sheet_name}': {e_update}", exc_info=True)
                    else:
                        logger.info(f"Заголовки на листе '{sheet_name}' уже корректны.")

        except gspread.exceptions.SpreadsheetNotFound:
            logger.error(f"Таблица Google с ID '{spreadsheet_id}' не найдена. "
                         f"Убедитесь, что ID корректен и у сервисного аккаунта есть доступ к этой таблице.")
        except Exception as e:
            logger.error(f"Ошибка при работе с таблицей ID '{spreadsheet_id}': {e}", exc_info=True)
