...
class PrivacyCache:
    def __init__(self):
        ...
        self._last_update = 0.0  # Инициализация как float
    ...
    def update_partial(self, new_users: Set[int], removed_users: Set[int]) -> None:
        ...
        self._last_update = time.time()
        logger.info(
            f"Privacy cache partially updated: added {len(new_users)} users, removed {len(removed_users)} users"
        )
...
class CatalogCache:
    ...
    def initialize(
        self, categories: List[Dict], items_by_category: Dict[str, List[Dict]]
    ) -> None:
        ...
        logger.info(
            f"Catalog cache initialized with {len(categories)} categories and {len(self._items_by_id)} items"
        )
    ...
    def update(
        ...
        self._last_sync_hash = self._calculate_hash()
        logger.info(
            f"Catalog cache updated with {len(categories)} categories and {len(self._items_by_id)} items"
        )
    ...
    def update_partial(
            ...
            if item_id in self._items_by_id:
                ...
                if category_name in self._items_by_category:
                    self._items_by_category[category_name] = [
                        i
                        for i in self._items_by_category[category_name]
                        if i["id"] != item_id
                    ]
        ...
        self._last_sync_hash = self._calculate_hash()
        logger.info(
            f"Catalog cache partially updated: added/updated {len(new_items)} items, removed {len(removed_item_ids)} items"
        )


class CartCache:
    def __init__(self):
        # user_id -> {item_id -> quantity (может быть int или float)}
        self._carts: Dict[int, Dict[str, Union[int, float]]] = defaultdict(dict)
        self._last_update = 0.0  # Инициализация как float
        self._update_interval = 3600

    def add_to_cart(
        self, user_id: int, item_id: str, quantity: Union[int, float]
    ) -> None:
        ...
        self._last_update = time.time()
        logger.info(
            f"Added/Updated {quantity} of item {item_id} to cart for user {user_id}"
        )

    def remove_from_cart(self, user_id: int, item_id: str) -> None:
        """Удаляет товар из корзины пользователя."""
        if user_id in self._carts and item_id in self._carts[user_id]:
            del self._carts[user_id][item_id]
            if not self._carts[user_id]:  # Если корзина пользователя стала пустой
                del self._carts[user_id]
            self._last_update = time.time()
            logger.info(f"Removed item {item_id} from cart for user {user_id}")

    def get_cart(self, user_id: int) -> Dict[str, Union[int, float]]:
    ...
    def get_item_quantity(self, user_id: int, item_id: str) -> Union[int, float]:
        """Возвращает количество товара в корзине пользователя."""
        return self._carts.get(user_id, {}).get(item_id, 0)
